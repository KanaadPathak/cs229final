...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  After comparing with VGG16 and VGG19, we choose ResNet50, since it uses Average Pooling before connect to the fully connected layers, which gives smaller feature set than VGG architectures.  We then download the pretrained weights from open source Keras framework.  \par
  See the first row of the filters extracted by the first Conv layer, they seem to represent the leaves, the outlines and the background respectively. The second row is from the activation layer where the one with highlights of the background has been filtered.
  %\vspace{0.3em}
